...
const graph_1 = require("../instantiation/graph");
...
class InstantiationService {
    ...
    invokeFunction(fn, ...args) {
        ...
        try {
            const accessor = {
                get: (id, isOptional) => {
                    if (_done) {
                        throw (0, ts_base_1.illegalState)('service accessor is only valid during the invocation of its target method');
                    }
    ...
    _createServiceInstance(ctor, args = [], _supportDelayedInstantiation, _trace) {
        if (!_supportDelayedInstantiation) {
            ...
            const idle = new ts_base_1.IdleValue(() => this._createInstance(ctor, args, _trace));
